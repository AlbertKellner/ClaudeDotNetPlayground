...
| CI/CD | GitHub Actions — workflows: Validar Execução (`ci.yml`: job único com steps sequenciais — Compilação (Native AOT) → Testes Unitários → Execução (Release) → Validação de Health Check e Endpoints), wiki-publish; GitHub Environment: ClaudeCode | — |
...
| 2026-03-19 | CI/CD consolidado: 5 jobs separados (Compilação, Execução, Teste Unitário, Health Check Debug, Health Check Publish) substituídos por job único com steps sequenciais — Build (Native AOT) → Testes Unitários → Execução Release → Validação de Endpoints; elimina checkouts e restores redundantes | Instrução do usuário |
