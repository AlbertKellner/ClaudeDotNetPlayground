...
| CI/CD | GitHub Actions — workflows: Validar Execução (`ci.yml`: Compilação → Execução → unit-tests → Validar Health Check (Debug) \|\| Validar Health Check (Publish)), Validar Execução Consolidado (`ci-consolidated.yml`: job único — Build AOT → Testes → Execução Release → Validação de Endpoints), wiki-publish; ambos os workflows de CI disparam em paralelo nos mesmos triggers; GitHub Environment: ClaudeCode | — |
...
| 2026-03-19 | CI/CD: novo workflow consolidado (`ci-consolidated.yml`) adicionado — job único com steps sequenciais (Build AOT → Testes → Execução Release → Validação de Endpoints); roda em paralelo com o workflow original (`ci.yml`) que permanece inalterado | Instrução do usuário |
