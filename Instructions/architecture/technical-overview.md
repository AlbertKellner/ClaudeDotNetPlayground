...
| Integração PokéAPI | Refit (`Refit.HttpClientFactory`) + Polly v8 (API pública, sem autenticação) | DA-022 |
...
| IPokeApi | `Shared/ExternalApi/Pokemon/IPokeApi.cs` | Interface Refit para a PokéAPI; contrato HTTP com rota `/api/v2/pokemon/{id}` |
| IPokeApiClient | `Shared/ExternalApi/Pokemon/IPokeApiClient.cs` | Interface de serviço; contrato que Features injetam via DI |
| PokeApiClient | `Shared/ExternalApi/Pokemon/PokeApiClient.cs` | Implementa IPokeApiClient; usa IPokeApi (Refit + Polly via HttpClient); aplica logging SNP-001 |
| CachedPokeApiClient | `Shared/ExternalApi/Pokemon/CachedPokeApiClient.cs` | Decorator de IPokeApiClient; implementa Memory Cache por usuário autenticado com duração configurável |
| PokeApiOutput | `Shared/ExternalApi/Pokemon/Models/PokeApiOutput.cs` | Modelo de resposta da PokéAPI (id, nome, altura, peso, tipos, sprites); inclui PokeApiJsonContext para AOT |
...
| 2026-03-21 | Integração PokéAPI adicionada: Refit + Polly; Shared/ExternalApi/Pokemon/ criada; Feature PokemonGet implementada; RN-009 | DA-022, RN-009 |
