...
A aplicação expõe um endpoint autenticado para consultar dados de um Pokémon por ID na PokéAPI. Neste primeiro momento, o ID está hardcoded com o valor correspondente ao Pikachu (25). A resposta é cacheada por usuário autenticado com duração configurável.

- **Endpoint:** `GET /pokemon`
- **Autenticação:** exigida — Bearer Token JWT válido no header `Authorization`
- **Comportamento:** consulta a PokéAPI (`GET /api/v2/pokemon/25`) com cache por usuário; retorna `HTTP 200` com id, nome, altura, peso, tipos e imagem frontal do Pokémon
