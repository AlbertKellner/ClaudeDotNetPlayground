# Log de Erros de Bash — Execução do Pipeline

Este arquivo documenta os erros encontrados durante a execução do pipeline de validação da integração com o Datadog, incluindo causa raiz e solução adotada.
...
| **Solução adotada** | Iniciar o daemon manualmente: `dockerd --host=unix:///var/run/docker.sock &>/tmp/dockerd.log &` e aguardar 5 segundos antes de prosseguir. |
...
| **Solução adotada** | Configurar o proxy no Docker via `~/.docker/config.json` com a seção `proxies`, passando `HTTP_PROXY`, `HTTPS_PROXY` e `NO_PROXY` do ambiente host. Após a configuração, os containers de build herdam automaticamente as configurações de proxy. |
...
| **Solução adotada** | Modificar o `Dockerfile` para aceitar um `ARG EXTRA_CA_CERT` (certificado CA em base64). Quando fornecido, decodifica e instala o certificado via `update-ca-certificates` antes do `dotnet restore`. Adicionar `EXTRA_CA_CERT` ao `.env` (base64 do arquivo `/usr/local/share/ca-certificates/swp-ca-production.crt`). Atualizar `docker-compose.yml` para passar o `args: EXTRA_CA_CERT: ${EXTRA_CA_CERT:-}`. |
...
| **Solução adotada** | Adicionar `DD_HOSTNAME=claudedotnetplayground-local` nas variáveis de ambiente do serviço `datadog-agent` no `docker-compose.yml`. |
...
| **Solução adotada** | Modificar o entrypoint para usar `cat /custom-certs/proxy-ca.crt >> /etc/ssl/certs/cacert.pem` (append ao bundle existente) antes de executar `/bin/entrypoint.sh`. |
...
| **Solução adotada** | Montar o arquivo de CA do host (`/usr/local/share/ca-certificates/swp-ca-production.crt`) no container via `volumes` e adicionar ao bundle `/etc/ssl/certs/cacert.pem` no entrypoint customizado. |
...
| **Solução adotada** | Adicionar `DD_CONVERT_DD_SITE_FQDN_ENABLED=false` nas variáveis de ambiente do `datadog-agent` no `docker-compose.yml`. Com isso, o agente usa hostnames sem ponto final, compatíveis com o proxy. |
